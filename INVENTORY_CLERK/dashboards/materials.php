...
?>
    <div class="page-stack materials-page">
        <section class="form-panel">
            <div class="materials-master-list__heading"><h2 class="section-title-inline">Material Master List</h2><div class="materials-master-list__controls"><nav class="materials-filter" aria-label="Material filter"><?php foreach (['active' => 'Active', 'inactive' => 'Archived', 'all' => 'All'] as $filterValue => $filterLabel): ?><a class="materials-filter__link<?php echo $materialFilter === $filterValue ? ' is-active' : ''; ?>" href="?material_filter=<?php echo htmlspecialchars($filterValue); ?>"><?php echo htmlspecialchars($filterLabel); ?></a><?php endforeach; ?></nav><button type="button" class="btn-primary materials-add-button" data-material-modal-open>+ Add Material</button></div></div>
    ...
    <div class="materials-modal" data-material-modal data-open-on-load="<?php echo $openMaterialModal ? 'true' : 'false'; ?>" data-material-draft-key="<?php echo htmlspecialchars($materialDraftKey); ?>" hidden>
        <section class="materials-modal__panel" role="dialog" aria-modal="true" aria-labelledby="addMaterialModalTitle">
            ...
            <form method="POST" data-material-form novalidate><input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($csrfToken); ?>">
                <div class="form-grid">
                    <div class="input-group materials-name-field"><label for="material_name">Material Name <span class="materials-suggestion" data-material-suggestion aria-live="polite"></span></label><div class="materials-name-field__control"><div class="materials-name-field__input-wrap"><input id="material_name" name="material_name" maxlength="180" required autocomplete="off" aria-autocomplete="list" aria-controls="materialNameSuggestions" aria-expanded="false" value="<?php echo htmlspecialchars($formValues['material_name']); ?>" data-material-name><div id="materialNameSuggestions" class="materials-name-suggestions" role="listbox" data-material-name-suggestions hidden></div></div><button type="button" class="materials-suggest-details" data-material-suggest-details>Suggest Details</button></div><div class="materials-name-field__message"><span class="materials-suggest-feedback" data-material-suggest-feedback aria-live="polite"></span><span class="materials-field-error" data-material-error="material_name" aria-live="polite"><?php echo htmlspecialchars((string)($formErrors['material_name'] ?? '')); ?></span></div></div>
